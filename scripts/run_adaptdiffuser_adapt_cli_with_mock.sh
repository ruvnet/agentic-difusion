...
# This script includes a built-in mock reward model implementation
...
echo "(with Built-in Mock Reward Model)"
echo "==============================================="
echo
...
if [ "$1" = "--device" ] && [ "$2" = "gpu" ]; then
  ...
  if check_gpu; then
    ...
    python scripts/test_adaptdiffuser_adapt_cli_with_mock.py --device gpu
  else
    echo "GPU requested but not available. Falling back to CPU."
    python scripts/test_adaptdiffuser_adapt_cli_with_mock.py --device cpu
  fi
elif [ "$1" = "--device" ] && [ "$2" = "both" ]; then
  echo "Running tests on both CPU and GPU (if available)..."
  python scripts/test_adaptdiffuser_adapt_cli_with_mock.py --device both
elif [ "$1" = "--test" ]; then
  echo "Running specific test: $2..."
  python scripts/test_adaptdiffuser_adapt_cli_with_mock.py --test $2
else
